...
def create_table() -> None:
	conn = sqlite3.connect(DATABASE)		# pass conn in instead
	c = conn.cursor()
...
try:
	...
	if conn is not None:
		...
		print("\nThe database already exists, with the following table{} : ".format("s" if len(tables) > 1 else ""))
		...
		response = str(input("\nWould you like to delete it and create a new one (y/n)? ")).lower()
